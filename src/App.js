...
function App() {
  const [mode, setMode] = useState('home'); // home, create, vote
  ...
  const loadElection = async (id) => {
    try {
      ...
      if (electionDoc.exists()) {
        setElection(electionDoc.data());
        setCandidates(electionDoc.data().candidates);
      } else {
  ...
  const removeCandidate = (id) => {
    ...
    setApprovedCandidates(new Set([...approvedCandidates].filter(cId => cId !== id)));
  };
